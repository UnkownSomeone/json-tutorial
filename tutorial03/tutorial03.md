...
> 很多时候我们都会自实现和st:vector相似的容器,例如RapidJSON里的Value中的Array和Objet也是用相同的线性结构。但我之前没留意一点,就是vector的增长因子(growth factor)。原始的HP STL实现的增长因子是2,即空间不足时把原来的容量增加一倍。我也沿用2这个常数于RapidJSON的实现里。
> 但昨天有德国网友(Kosta) 指出，这并不是一-个好选择，他弓|用Folly FBVector实现的说明(folly/FBVector.md at master . facebook/folly . GitHub)，内里谈及,如果增长因子是2,那么每次重新分配时,必然不能用到之前分配的地址空间(因为2^n> 2^0 + 2^1 + ..2^2(0-1)) ,在缓存上不友好。所以建议增长因子最好是少于2,例如是1.5。
>
> 我们假设有一段连续的内存空间，并且使用增长因子为1.2来动态分配内存。初始时，我们分配了一个长度为10的内存块，起始地址为0。
>
> 第一次分配10个元素后，我们使用了地址范围从0到9。接下来，当我们需要更多的空间时，根据增长因子1.2，我们将分配一个长度为12的内存块，起始地址为10。现在我们使用的地址范围为10到21。
>
> 第三次分配时，根据增长因子1.2，我们需要分配一个长度为15的内存块。然而，我们注意到从0到21之间的地址空间已经被使用了，所以我们不得不分配一个新的内存块。这个新的内存块的起始地址是22。现在，我们使用的地址范围为22到36。
>
> 第四次分配时，根据增长因子1.2，我们需要分配一个长度为18的内存块。同样地，由于地址空间22到36已经被使用，我们需要分配一个新的内存块。这个新的内存块的起始地址可以回到0，因为在这个地址范围内现在都是空闲的。现在，我们使用的地址范围是0到17。
>
> 总结来说，如果增长因子小于2，比如1.2，在分配内存时我们会更加高效地利用已经分配的地址空间。而如果增长因子大于等于2，就会出现每次都需要分配新的内存块，导致无法重复利用之前分配的地址空间。这样的情况在缓存方面可能不太友好，因为无法有效地利用缓存中已经加载的数据。
